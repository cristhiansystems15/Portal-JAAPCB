...
function render(data){
 ...
 const estadoNormalizado=estadoTexto.toUpperCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').trim();
 ...
 const reunionesPublicadas=
   Array.isArray(data.reuniones)?data.reuniones:
   (Array.isArray(data.reunionesPublicadas)?data.reunionesPublicadas:
   (Array.isArray(data.reunionesCalendario)?data.reunionesCalendario:[]));
 ...
 detalle.forEach(x=>{
   ...
   const yes=rawAsistencia===true||['TRUE','VERDADERO','SI','SÍ','ASISTIO','ASISTIÓ'].includes(String(rawAsistencia).trim().toUpperCase());
 ...
 renderList('reunionesLista',reunionesPublicadas,x=>addItem($('reunionesLista'),x.descripcion||x.titulo||x.nombre||'Reunión de abonados',x.fecha||x.fechaReunion||x.date,(x.lugar||x.ubicacion||'')+' · '+(x.hora||x.horaReunion||'')),'No hay reuniones publicadas.');
 ...
 $('salir').classList.remove('hidden');$('menuBtn').classList.remove('hidden');
 $('sidebar').classList.add('opened');go('inicio');
 document.querySelector('.topbar').scrollIntoView({behavior:'smooth',block:'start'});
}

async function consultar(){
 const id=formatIdentidad($('identidad').value.trim());$('identidad').value=id;
...
function salir(){
 ...
 $('menuBtn').classList.add('hidden');$('sidebar').classList.remove('opened','open');$('menuBackdrop').classList.remove('show');
...
function go(target){
 ...
 const targetEl=document.getElementById(realTarget);if(targetEl)targetEl.classList.add('active');
...
function toggleMenu(){
 const side=$('sidebar'),back=$('menuBackdrop'),mobile=window.innerWidth<=720;
 if(mobile){side.classList.toggle('open');back.classList.toggle('show');}
 else{side.classList.toggle('opened');}
}
function closeMenu(){$('sidebar').classList.remove('open');$('menuBackdrop').classList.remove('show');}
